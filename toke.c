...
S_scan_const(pTHX_ char *start)
{
	...
	if (*s == '\\' && s+1 < send) {
		...
		*--s = '$';
	     ...
	     * charname it must be followed immediately by a '{' */
		...
		*d++ = NATIVE_TO_NEED(has_utf8,'\\');
		 ...
		 * pattern, or ambiguous inside.  If there is a '}' in a
		 * pattern, it could also be a curly quantifier, like \N{3,}
		 * meaning to match 3 or more non-newlines.  regcurly returns a
		 * boolean indicating if it is a legal quantifier */
		if (!e || (PL_lex_inpat && regcurly(s))) {
		    if (! PL_lex_inpat) {
			...
			/* Here, is inside a pattern and is either ambiguous or
			 * has a legal quantifier.  We resolve any ambiguity by
			 * assuming it is a "match non-newline".  This is
			 * because that is a legal interpretation of the input
			 * and the other isn't. */
			if (! e && ckWARN(WARN_SYNTAX)) {
			    Perl_warner(aTHX_ packWARN(WARN_SYNTAX),
