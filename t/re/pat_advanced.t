...
sub run_tests {
  ...
  SKIP:
    ...
    BEGIN {
        ...
        ok $w && $w =~ /Using just the first bytes returned/, 'Verify that got too-long string warning in \N{} that exceeds the limit';
        undef $w;
        eval q [ok "\N{LONG-STR}" =~ /^\N{TOO-LONG-STR}$/i, 'Verify under folding that too long a string fails gracefully'];
        ok $w && $w =~ /Using just the first bytes returned/, 'Verify under folding that got too-long string warning in \N{} that exceeds the limit';
        undef $w;
        ...
        ok $w && $w =~ /Using just the first bytes returned/i, 'Verify under folding that got too-long string warning in \N{} that exceeds the limit';
        undef $w;
