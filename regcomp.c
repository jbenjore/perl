...
/* reg_namedseq(pRExC_state,UVp)
   ...
   to match a non-newline.  The tokenizer has attempted to decide which, and in
   the case of a named sequence converted it into one of the forms: \N{} (if
   the sequence is null), or \N{U+c1.c2...}, where c1... are the characters in
   the sequence.  That means that if the beginning of the passed-in pattern
   isn't '{U+' or there is no '}', it signals that this \N occurrence means to
   match a non-newline.  Only the \N{U+...} form should occur in a character
   class, for the same reason that '.' inside a character class means to just
   match a period: it just doesn't make sense.
...
S_reg_namedseq(pTHX_ RExC_state_t *pRExC_state, UV *valuep, I32 *flagp)
{
	...
	*valuep = grok_hex(RExC_parse, &length_of_hex, &flags, NULL);
	...
	/* The tokenizer should have guaranteed validity; but check anyway */
	...
	if (SIZE_ONLY) {
		...
		ckWARNreg(RExC_parse - 1,
			  "Using just the first bytes returned by \\N{}");
	    }
