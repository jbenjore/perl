...
#endif
    int old_in_utilize = PL_in_utilize;
    PL_in_utilize = 1;
...
STATIC int
S_strict_classes_p(pTHX)
{
	return 1;

	
	SV *lex_flag = Perl_refcounted_he_fetch(aTHX_ PL_curcop->cop_hints_hash,
											0, "classes", 7, 0, 0);
	if ( ! lex_flag
		 || ! SvOK( lex_flag ) ) {
		return 1;
	...
	return SvTRUE( lex_flag );

}


STATIC void
S_ck_strict_classes(pTHX_ OP *invocant_op)
{
    HV *invocant_stash;
    SV *invocant_sv;
    char *invocant_pv;
    STRLEN invocant_pv_len;

    PERL_ARGS_ASSERT_CK_STRICT_CLASSES;

    if ( invocant_op->op_type == OP_CONST
	 && strict_classes_p()
	 && ! PL_in_utilize ) {
	
	/* Fetch the class name.
	 *
	 * TODO: what happens when the class name is utf8?
	 */
	invocant_sv = cSVOPx_sv( invocant_op );
	assert( invocant_sv );
	invocant_pv = SvPV( invocant_sv, invocant_pv_len );
	/* Fetch the class */
	invocant_stash = gv_stashpvn( invocant_pv, strlen(invocant_pv), 0 );

	if ( NULL == invocant_stash ) {
	    /* TODO: Perl_mess. TODO: refactor the exception to
	     * separate S_ function?
	     */
	    PerlIO_printf(PerlIO_stderr(),
			  "No such class \"%s\"",
			  invocant_pv);
	    Perl_croak(aTHX_ "No such class \"%s\"",
		       invocant_pv);
	}
    }
}
...
STATIC void
S_ck_strict_methods(pTHX_ OP *invocant_op, OP *method_op)
{
    dSP;
    SV *method_sv, *invocant_sv, *found_sv;
    HV *stash;
    char *stash_name, *variable_name, *method_name;
    int found_method;
    I32 count;

    PERL_ARGS_ASSERT_CK_STRICT_METHODS;

    /* TODO: should I handle invocant_op->op_type == OP_LIST? */
    if ( !( ! PL_in_utilize
	    && ( invocant_op->op_type == OP_PADSV
		 || invocant_op->op_type == OP_CONST )
    ...
    /* Fetch class name */
    if ( invocant_op->op_type == OP_PADSV ) {

	/* Fetch the object's class from the PADLIST */
	stash = PAD_COMPNAME_TYPE( invocant_op->op_targ );
	if ( NULL == stash ) {
	    return;
	}

	/* Fetch the class name from the stash */
	stash_name     = HvNAME( stash );
	if ( NULL == stash_name ) {
	    return;
	}
	
	/* Set up for freeing any other temporaries generated by
	 * ->can()
	 */
	ENTER;
	SAVETMPS;

	/* Create the class name as a perl string and make it a
	 * temporary
	 */
	invocant_sv = newSVpv( stash_name, 0 );
	sv_2mortal( invocant_sv );
    }
    else if ( invocant_op->op_type == OP_CONST ) {

	/* Fetch the class name from the optree */
	invocant_sv = cSVOPx_sv( invocant_op );
	if ( NULL == invocant_sv ) {
	    return;
	}
	stash_name = SvPV_nolen( invocant_sv );


	/* Set up for freeing the invocant_sv temporary and any other
	 * temporaries generated by ->can()
	 */
	ENTER;
	SAVETMPS;
    }
    ...
    /* Call ->can() Allowing exceptions to bubble up out of here. */
    count = call_method( "can", G_SCALAR );

    /* Get the result of ->can() */
    SPAGAIN;
    found_sv = POPs;
    found_method = count && SvTRUE(found_sv);
    ...
    method_name   = SvPV_nolen(method_sv);
    if ( invocant_op->op_type == OP_PADSV ) {
	variable_name = PAD_COMPNAME_PV( invocant_op->op_targ );

	/* TODO: Perl_mess. TODO: can there be nulls in the method name?
	 * If not, I can use SvPV_nolen.
	 */
	PerlIO_printf(PerlIO_stderr(),
		      "No such method \"%s\" for variable %s of type %s",
		      method_name, variable_name, stash_name);
	Perl_croak(aTHX_
		   "No such method \"%s\" for variable %s of type %s",
		   method_name, variable_name, stash_name);
    }
...
Perl_ck_subr(pTHX_ OP *o)
{
	...
	if (o2->op_type == OP_CONST) {
	    ...
	    ck_strict_classes( o2 );
	    ck_strict_methods( o2, cvop );
	}
	    ...
	    ck_strict_classes( sib );
	    ck_strict_methods( sib, cvop );
	}
	else {
	    ck_strict_classes( o2 );
	    ck_strict_methods( o2, cvop );
